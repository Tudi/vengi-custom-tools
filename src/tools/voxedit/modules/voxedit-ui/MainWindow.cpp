...
bool MainWindow::init() {
	...
	_numViewports = core::getVar(cfg::VoxEditViewports);
	_tipOfTheDay = core::getVar(cfg::VoxEditTipOftheDay);
	_popupTipOfTheDay = core::getVar(cfg::VoxEditPopupTipOfTheDay);
	_popupWelcome = core::getVar(cfg::VoxEditPopupWelcome);
	_popupMinecraftMapping = core::getVar(cfg::VoxEditPopupMinecraftMapping);
...
void MainWindow::popupMinecraftMapping() {
	...
	if (ImGui::BeginPopupModal(title.c_str(), nullptr, ImGuiWindowFlags_NoSavedSettings)) {
		...
		if (ImGui::BeginTable("##minecraftmapping", 2, TableFlags, outerSize)) {
			...
			for (size_t i = 0; i < minecraftPaletteMap.size(); ++i) {
...
void MainWindow::updateViewMode() {
	if (viewModePaletteFormat6Bit(_viewMode->intVal())) {
		core::getVar(cfg::PalformatRGB6Bit)->setVal(true);
	} else {
		core::getVar(cfg::RenderNormals)->setVal(false);
	}

	const ViewMode vMode = (ViewMode)_viewMode->intVal();
	if (ViewMode::AceOfSpades == vMode) {
		core::getVar(cfg::VoxEditMaxSuggestedVolumeSize)->setVal(512);
	} else {
		core::getVar(cfg::VoxEditMaxSuggestedVolumeSize)->setVal(128);
	}
