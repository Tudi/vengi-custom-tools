...
app::AppState WindowedApp::onConstruct() {
	...
	const core::VarDef renderCheckerBoard(cfg::RenderCheckerBoard, false, N_("Checkerboard"), _("Activate checkerboard rendering"), core::CV_SHADER);
	core::Var::registerVar(renderCheckerBoard);
	const core::VarDef renderCullBuffers(cfg::RenderCullBuffers, false, N_("Cull buffers"), _("Activate culling for buffer parts"));
	core::Var::registerVar(renderCullBuffers);
	const core::VarDef renderCullNodes(cfg::RenderCullNodes, true, N_("Cull nodes"), _("Activate culling for scene nodes"));
	...
	const core::VarDef clientDebugShadow(cfg::ClientDebugShadow, false, N_("Debug shadows"), _("Activate shadow debug rendering"), core::CV_SHADER);
	...
	const core::VarDef clientDebugShadowMapCascade(cfg::ClientDebugShadowMapCascade, false, N_("Debug cascades"), _("Activate cascade shadow map debug rendering"), core::CV_SHADER);
	core::Var::registerVar(clientDebugShadowMapCascade);
	const core::VarDef clientGamma(cfg::ClientGamma, 1.0f, 0.1f, 4.0f, N_("Gamma"), _("Gamma correction"), core::CV_SHADER);
	...
	const core::VarDef clientOpenGLVersion(cfg::ClientOpenGLVersion, "3.3", N_("OpenGL Version"), _("The OpenGL version to use"), core::CV_READONLY);
	...
	const core::VarDef clientDebugSeverity(cfg::ClientDebugSeverity, 0, N_("Debug severity"), _("0 disables it, 1 only highest severity, 2 medium severity, 3 everything"));
	core::Var::registerVar(clientDebugSeverity);
	const core::VarDef clientMouseRotationSpeed(cfg::ClientMouseRotationSpeed, 0.01f, N_("Rotation speed"), N_("The rotation speed when using the mouse to rotate the camera"));
	...
	const core::VarDef clientCameraMinZoom(cfg::ClientCameraMinZoom, 0.001f, 0.0001f, 1000.0f, N_("Min zoom"), _("The minimum zoom level for the camera"), 0u);
...
void WindowedApp::centerMousePosition() {
	SDL_WarpMouseInWindow(_window, frameBufferWidth() / 2.0f, frameBufferHeight() / 2.0f);
}
