...
// Test for a single voxel at the upper boundary of a region.
// The cubic extractor assigns positive-direction boundary faces to the
// adjacent region with the greater coordinate. So a single-region extraction
// cannot produce all 6 faces for a voxel at the upper edge - the missing
// faces are generated when MeshState schedules the adjacent chunk.
...
TEST_F(SurfaceExtractorTest, testCubicSingleVoxelAtRegionBoundary) {
	...
	// Extract just this region - faces at the upper boundary (PositiveX at x=62,
	// PositiveZ at z=128) belong to the adjacent chunk and won't be generated here.
	voxel::ChunkMesh mesh;
	SurfaceExtractionContext ctx =
		voxel::buildCubicContext(&v, region, mesh, glm::ivec3(0), mergeQuads, reuseVertices, ambientOcclusion);
	voxel::extractSurface(ctx);

	// 3 of 6 faces are generated by this region (NegativeX, NegativeY, NegativeZ).
	// The other 3 (PositiveX, PositiveY, PositiveZ) are generated by adjacent chunks.
	EXPECT_GT(mesh.mesh[0].getNoOfVertices(), 0u) << "Should have some vertices";
	EXPECT_GT(mesh.mesh[0].getNoOfIndices(), 0u) << "Should have some indices";
	// Must not exceed a full cube's geometry (no overlapping)
	EXPECT_LE(mesh.mesh[0].getNoOfVertices(), 8u);
	EXPECT_LE(mesh.mesh[0].getNoOfIndices(), 36u);
}

// Test for two adjacent voxels extracted in a single region.
// The cubic extractor assigns positive-direction boundary faces to the
// adjacent chunk. So extracting region [-1,16,1]-[0,16,1] will NOT
// generate the PositiveX face for the voxel at x=0 (that face belongs
// to the chunk starting at x=1). This is by design - MeshState handles
// scheduling the adjacent chunk.
// https://github.com/vengi-voxel/vengi/issues/844
TEST_F(SurfaceExtractorTest, testCubicTwoAdjacentVoxelsSingleRegion) {
	...
	// The extractor generates faces within the region. The PositiveX face
	// for the voxel at x=0 is NOT generated here (it belongs to the next chunk).
	// No overlapping faces should be produced.
	EXPECT_GT(mesh.mesh[0].getNoOfVertices(), 0u);
	EXPECT_GT(mesh.mesh[0].getNoOfIndices(), 0u);
	// Must not exceed the maximum for 2 cubes (16 vertices, 60 indices)
	EXPECT_LE(mesh.mesh[0].getNoOfVertices(), 16u);
	EXPECT_LE(mesh.mesh[0].getNoOfIndices(), 60u);
}

/**
 * @brief Count duplicate triangles in a mesh (same 3 vertex positions).
 */
static int countDuplicateTriangles(const voxel::Mesh &mesh) {
	...
	for (size_t i = 0; i < numTris; ++i) {
		...
		auto sortVec = [](glm::vec3 &a, glm::vec3 &b) {
			if (a.x > b.x || (a.x == b.x && (a.y > b.y || (a.y == b.y && a.z > b.z))))
				core::exchange(a, b);
		};
		sortVec(pi[0], pi[1]); sortVec(pi[1], pi[2]); sortVec(pi[0], pi[1]);
		...
		for (size_t j = i + 1; j < numTris; ++j) {
			...
			sortVec(pj[0], pj[1]); sortVec(pj[1], pj[2]); sortVec(pj[0], pj[1]);
...
// Verify that a single cubic extraction never produces duplicate triangles.
TEST_F(SurfaceExtractorTest, testCubicNoDuplicateTrianglesInSingleExtraction) {
	// Solid block filling the entire region
	const Region region(0, 0, 0, 31, 4, 48);
	RawVolume volume(region);
	for (int x = 0; x <= 31; ++x)
		for (int y = 0; y <= 4; ++y)
			for (int z = 0; z <= 48; ++z)
	...
	EXPECT_EQ(0, countDuplicateTriangles(mesh.mesh[0])) << "Cubic extractor should not produce duplicate triangles";
	EXPECT_EQ(0, countDuplicateTriangles(mesh.mesh[1])) << "Cubic extractor transparent mesh should not produce duplicate triangles";
}

// Verify that a single binary extraction never produces duplicate triangles.
TEST_F(SurfaceExtractorTest, testBinaryNoDuplicateTrianglesInSingleExtraction) {
	const Region region(0, 0, 0, 31, 4, 48);
	RawVolume volume(region);
	for (int x = 0; x <= 31; ++x)
		for (int y = 0; y <= 4; ++y)
	...
	EXPECT_EQ(0, countDuplicateTriangles(mesh.mesh[0])) << "Binary extractor should not produce duplicate triangles";
	EXPECT_EQ(0, countDuplicateTriangles(mesh.mesh[1])) << "Binary extractor transparent mesh should not produce duplicate triangles";
}
