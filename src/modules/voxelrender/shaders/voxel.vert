...
void main(void) {
	...
	if (u_gray != 0 || (u_has_selection != 0 && (a_flags & FLAGOUTLINE) == 0u)) {
		...
		// blue tint: reduce red/green to visually distinguish marked nodes
		const float markedTintR = 0.6;
		const float markedTintG = 0.8;
		const float markedTintB = 1.0;
		v_color = vec4(materialColor.r * markedTintR, materialColor.g * markedTintG, materialColor.b * markedTintB, materialColor.a);
	} else {
